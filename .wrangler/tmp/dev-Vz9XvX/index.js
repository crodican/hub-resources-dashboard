...
var serveStatic = /* @__PURE__ */ __name(() => {
  return async (c) => {
    const env2 = c.env;
    const res = await env2.ASSETS.fetch(c.req.raw);
    if (res.status === 404) {
      return c.notFound();
    }
    return res;
...
var parseGoogleSheetsCSV = /* @__PURE__ */ __name((csvText) => {
...
var generateGoogleSheetsStyleCSV = /* @__PURE__ */ __name((records) => {
  if (records.length === 0) {
  ...
  const dataRows = records.map((record) => {
    return headers.map((header) => {
      ...
      if (stringValue.includes(",") || stringValue.includes('"') || stringValue.includes("\n") || stringValue.includes("\r") || stringValue.startsWith(" ") || stringValue.endsWith(" ") || stringValue === "") {
...
api.get("/filters", async (c) => {
  try {
    ...
    `).all();
    const categoryMap = {};
    categoriesByType.results.forEach((row) => {
    ...
    const filterData = {
      "County": counties.results.map((c2) => c2.county_name),
      "Resource Type": resourceTypes.results.map((rt) => rt.resource_type_name),
      "Populations Served": populations.results.map((p) => p.population_name),
    ...
    console.error("Error fetching filters:", error3);
    return c.json({
      success: false,
...
api.get("/", async (c) => {
  try {
    ...
    console.error("Error fetching resources:", error3);
    return c.json({
      success: false,
...
api.get("/map-markers", async (c) => {
  try {
    ...
    const markers = (results.results || []).map((record) => {
    ...
    console.error("Error in map markers:", error3);
    return c.json({
      success: false,
...
api.get("/stats", async (c) => {
  try {
    ...
    console.error("Error in stats:", error3);
    return c.json({
      success: false,
...
api.get("/export", async (c) => {
  ...
  try {
    ...
    console.error("Error in export:", error3);
    return c.json({
      success: false,
...
api.get("/whoami", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in whoami:", error3);
...
api.post("/insert", authMiddleware, async (c) => {
  try {
    ...
    const requiredFields = ["location_name", "organization_name"];
    ...
    console.error("Error in insert:", error3);
    return c.json({
      success: false,
      error: { message: `Insert failed: ${error3.message}` }
    }, 500);
  }
});
api.put("/update", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in update:", error3);
    return c.json({
      success: false,
      error: { message: `Update failed: ${error3.message}` }
    }, 500);
  }
});
api.delete("/delete", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in delete:", error3);
    return c.json({
      success: false,
      error: { message: `Delete failed: ${error3.message}` }
    }, 500);
  }
});
api.post("/sync", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in sync:", error3);
    return c.json({
      success: false,
      error: { message: `Sync failed: ${error3.message}` }
    }, 500);
  }
});
api.post("/webhook/sync", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in webhook sync:", error3);
    return c.json({
      success: false,
      error: { message: `Webhook sync failed: ${error3.message}` }
    }, 500);
  }
});
  ...
  try {
    ...
    for (const record of data) {
      try {
        ...
        console.error(`Error inserting record:`, record, error3);
        errorCount++;
      }
    }
    return c.json({
    ...
    console.error("Error in import:", error3);
    return c.json({
      success: false,
      error: { message: `Import failed: ${error3.message}` }
    }, 500);
  }
});
api.post("/sql", authMiddleware, async (c) => {
  try {
    ...
    console.error("Error in SQL:", error3);
    return c.json({
      success: false,
      error: { message: `SQL execution failed: ${error3.message}` }
    }, 500);
  }
});
var api_default = api;
...
// .wrangler/tmp/bundle-UL1vaH/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UL1vaH/middleware-loader.entry.ts
